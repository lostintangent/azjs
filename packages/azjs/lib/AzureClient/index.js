...
module.exports = class AzureClient {
  addLogpoint(file, line, expression, linux = false) {
    ...
    const command = linux
      ? `node webSSHClient.js ${file} ${line} ${expression}`
      : `curl -s "http://localhost:8967/add?file=${file}&line=${line}&expression=${escape(
          expression
        )}"`;
  ...
  createResourceGroup() {
    return runOperation({
      ...
      operation: () =>
        this.client.resourceGroups.createOrUpdate(this.resourceGroup, {
  ...
  createWebApp(enableGit, remoteRepoUrl, linux) {
    return runOperation({
      ...
      operation: () => {
        ...
        let templateFile = linux
          ? "linuxTemplate.json"
          : remoteRepoUrl ? "gitTemplate.json" : "template.json";
  ...
  createConfig(linux) {
    return startScript().then(startScript => {
      ...
      if (linux) {
        fse.copySync(
          path.join(__dirname, "templates", "process.json"),
          path.join(process.cwd(), "process.json")
        );
        fse.copySync(
          path.join(__dirname, "templates", "webSSHClient.js"),
          path.join(process.cwd(), "webSSHClient.js")
        );
      } else {
        fse.copySync(
          path.join(__dirname, "templates", "web.config"),
    ...
    return runOperation({
      ...
      operation: () => {
        return this.createConfig(linux).then(clearConfig => {
          return this.kuduClient.uploadDirectory().then(clearConfig);
        });
      }
    });
  }
  ...
  exportResourceGroup() {
    return this.client.resourceGroups
      .exportTemplate(this.resourceGroup, { resources: ["*"] })
      .then(({ template }) => template);
  }

  getGitRemote() {
    return this.appClient.webApps
      .listPublishingCredentials(this.resourceGroup, this.appName)
      .then(({ publishingUserName, publishingPassword }) => {
        // TODO: How can I retrieve this entire URL as opposed to needing to construct it
        return `https://${publishingUserName}:${publishingPassword}@${this
          .appName}.scm.azurewebsites.net/${this.appName}.git`;
      });
  }
    ...
    return runOperation({
      ...
      operation: () =>
        this.runRemoteCommand(
          "npm install applicationinsights logpoint --no-save"
        )
    });
  }
  ...
  login() {
    ...
    return login({
      serviceName: `${pkg.name}/${pkg.version}`
  ...
  restartApp() {
    return runOperation({
      ...
      operation: () =>
        this.appClient.webApps.restart(this.resourceGroup, this.appName, {
          synchronous: true
        })
    });
  }
  ...
  syncRemoteRepo() {
    return runOperation({
      ...
      operation: () =>
        this.appClient.webApps.syncRepository(this.resourceGroup, this.appName)
    });
  }
};
