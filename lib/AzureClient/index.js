...
module.exports = class AzureClient {
    ...
    buildAppIfNeeded() {
        const buildScript = this.getPackageScript("build");
        if (!buildScript) {
        ...
        return runOperation({
            ...
            operation: () => {
                // Install the dev dependencies, build the app and then
                // clean the dev dependencies from the machine.
                return this.runRemoteCommand("npm install --only=dev").then(() => {
                    return this.runRemoteCommand("npm run build");
                });
            }
        });
    }
    ...
    installDependencies() {
        return runOperation({
            ...
            operation: () => this.runRemoteCommand("npm install --production")
        });
    }
    ...
    installAppInsights() {
        return runOperation({
            ...
            operation: () => {
                return this.runRemoteCommand("npm install applicationinsights").then(() => {
                    return this.runRemoteCommand("npm install logpoint");
                });
            }          
        });
    }
    ...
    login() {
        ...
        return login({ serviceName: `${pkg.name}/${pkg.version}` })
            .then(({ request, clientFactory, subscriptionId }) => {
